...
//#include <KICK_AT.h>
//#include <KICK2_AT.h>
//#include "bastlAttinySamples/RIDE_AT.h"
//#include <SNARE_AT.h>
//#include <SNARE2_AT.h>
//#include <TR_CB_AT.h>
...
//#include <CHEB4.h>
#define RIDE_INDEX 0
#define SNARE_INDEX 1
#define KICK_INDEX 2
const char* sampleTable[]={TR_HH_DATA,SNARE2_DATA,TR_KICK_DATA};
const uint16_t sampleLength[]={TR_HH_NUM_CELLS,SNARE2_NUM_CELLS,TR_KICK_NUM_CELLS};
...
void setLength(uint8_t _length){
  sampleEnd=map(_length,0,255,0,sampleLength[incr]);
}
bool render;
int index;
bool reverse=false;
...
ISR(TIMER1_COMPA_vect)  // render primary oscillator in the interupt
{
  ...
  /*
  index++;
  if(index>=sampleEnd){analogValues[WS_2]<<2){
    index=0;
  }
  */
  out = (char)pgm_read_byte_near(sampleTable[incr]+index)+128;
 // out=(out*decayVolume)>>8;
 // out=out&analogValues[WS_2];
  //out+=128;
  out=(out*decayVolume)>>8;
   OCR0A= out;
  // }
...
bitWrite(PORTB,PINB1,1);
  ...
  if(newState&& !resetState) decayVolume=255, index=0, render=true;// t=0;
   if(!newState&& resetState) decayVolume=255;
  resetState=newState;
  gate=newState;
  
 if(clocks()-time>(analogValues[WS_2]<<5)){
   time=clocks(); 
   if(!gate){
   if(decayVolume>0) decayVolume--; 
  }
  if(gate) decayVolume=255;
    /*
 if(debounceCounter>3){
  ...
  /*
  if(clocks()-time>((255-analogValues[WS_2])<<3)){ // render secondary oscillator in the loop
   time=clocks(); 
...
ISR(ADC_vect){ // interupt triggered ad completion of ADC counter
  if(!firstRead){ // discard first reading due to ADC multiplexer crosstalk
    ...
    if(lastAnalogChannelRead==PITCH && lastAnalogValues[PITCH]!=analogValues[PITCH]) setFrequency(constrain(mapLookup[analogValues[PITCH]]<<2,0,940));
